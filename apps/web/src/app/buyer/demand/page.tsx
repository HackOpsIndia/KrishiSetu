...
export default function BuyerDemandPage() {
  ...
  const demands = [
    {
      id: 'demand-freshmart-tomato',
      commodity: 'Tomato',
      variety: 'Hybrid',
      grade: 'A',
      totalQuantityQtl: 200,
      fulfilledQuantityQtl: 75,
      minLotSizeQtl: 10,
    ...
    {
      id: 'demand-freshmart-onion',
      commodity: 'Onion',
      variety: 'Garwa / Red',
      grade: 'A',
      totalQuantityQtl: 500,
      fulfilledQuantityQtl: 500,
      minLotSizeQtl: 50,
      targetPriceRange: '₹2,400 – ₹2,600/qtl',
      deliveryLocation: 'Chakan Processing Unit, Pune',
      providesPickup: false,
      pickupRadiusKm: 100,
      paymentTerm: 'T+2 Bank Settlement',
      status: 'FULFILLED',
  ...
  const matchedSupplies = [
    ...
    },
    {
      ...
      grade: 'A',
      matchScore: 95,
      status: 'Eligible for Direct Offer',
    },
    {
      lotId: 'lot-meena-30qtl',
      farmerName: 'Meena Deshpande',
    ...
    {
      lotId: 'fpo-cluster-75qtl',
      farmerName: 'Pune FPO Collective',
      village: 'Haveli Consolidation Hub',
      distanceKm: 38,
      quantityQtl: 75,
      grade: 'A',
      matchScore: 99,
      status: 'Pooled Shipment Ready',
    },
  ];
              ...
              <div className="space-y-3 flex-1">
                <div className="flex items-center gap-3">
                  ...
                  <h4 className="text-xl font-bold text-neutral-900">
                    {d.commodity} ({d.variety}) — Grade {d.grade}
                  </h4>
                </div>

                <p className="text-xs text-neutral-600">
                  Target Price: <strong>{d.targetPriceRange}</strong> • Min Lot: {d.minLotSizeQtl} Qtl • {d.providesPickup ? 'Direct Company Pickup' : 'Delivery at Plant'} ({d.pickupRadiusKm} km radius)
                </p>
                ...
                <div className="space-y-1 pt-1 max-w-xl">
                  <div className="flex justify-between text-xs text-neutral-600 font-medium">
                    ...
                    <span>{d.fulfilledQuantityQtl} / {d.totalQuantityQtl} Qtl ({Math.round((d.fulfilledQuantityQtl / d.totalQuantityQtl) * 100)}%)</span>
                  </div>
                  <div className="w-full h-2.5 bg-neutral-200 rounded-full overflow-hidden flex">
                    <div
                      className="bg-emerald-500 h-full"
                      style={{ width: `${(d.fulfilledQuantityQtl / d.totalQuantityQtl) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
                ...
                {matchedSupplies.map((item) => (
                  <tr key={item.lotId} className="hover:bg-neutral-50/60">
                    ...
                    <td className="py-3 px-4 text-right">
                      <Link
                        href="/buyer/offers"
                        className="text-[#ef4d23] hover:underline font-bold"
                      >
                        Negotiate →
                      </Link>
                    </td>
          ...
          <div className="mt-6 pt-6 border-t border-neutral-200 grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="p-3.5 rounded-2xl bg-neutral-50/70 border border-neutral-200">
              ...
              <p className="text-neutral-500 text-[11px] leading-relaxed">
                Min Brix 4.5, Firmness ≥ 85%, transit decay threshold &le; 2.0%. Geotagged inspection verified on farm gate dispatch.
              </p>
            </div>
            <div className="p-3.5 rounded-2xl bg-neutral-50/70 border border-neutral-200">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl border border-neutral-200">
            ...
            >
              <div>
                ...
                <select className="w-full border border-neutral-200 rounded-xl px-3 py-2 text-xs bg-neutral-50">
